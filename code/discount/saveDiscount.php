...
try {
    ...
    if ($check_result->num_rows > 0) {
        ...
        $update_stmt->bind_param("dddddi", $price_discount, $shipping_discount, $fee_credit, $tax_return, $total_discount, $id_sell);
        ...
        // Insert new record
        $insert_sql = "INSERT INTO discounts (id_sell, sell_order, price_discount, shipping_discount, fee_credit, tax_return, total_discount, created_at, updated_at) 
                       VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
        
        $insert_stmt = $mysqli->prepare($insert_sql);
        $insert_stmt->bind_param("isddddd", $id_sell, $sell_order, $price_discount, $shipping_discount, $fee_credit, $tax_return, $total_discount);
